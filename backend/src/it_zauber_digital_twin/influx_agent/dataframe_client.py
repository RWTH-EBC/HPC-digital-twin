# -*- coding: utf-8 -*-
"""
This is an almost identical copy to the influxd.client.DataFrameClient class
from the influxdb package (https://github.com/influxdata/influxdb-python/tree/master)

The repository is archived and there is one necessary change that has to be made
to the DataFrameClient class, therefore it is copied here. In the future, one could
also think about forking the original repo
"""
...
from influxdb.line_protocol import _escape_tag
...
class DataFrameClient(InfluxDBClient):
    ...
    def __init__(self, *args, **kwargs):
        ...
        super().__init__(*args, host=host, port=port, database=database, **kwargs)
    ...
    def query(
        ...
        results = super().query(query, **query_args)
    ...
    def _to_dataframe(self, rs, dropna=True, data_frame_index=None):
        ...
        for key, data in rs.items():
            ...
            # This is the only line that changes, giving the format
            # allows for creation of the dataframe when there are different
            # timestamps in the database, which all comply with the iso8601 standard
            # but are different. not giving the format will result in an error and
            # since the database gets data from different sources, the timestamps
            # will (and already do) surely have different formats
            #
            # As of 28.11.23 this is not accurate anymore for some reason, now working without
            # format. Will have to check at some point #TODO
